...
/// <summary>
/// An abstract base class for a saga step that is also a MediatR command.
...
public abstract class MediatRTransactionalSagaStep<TContext, TRequest, TResponse> : TransactionalSagaStep<TContext>, ITransactionalSagaStep<TContext>
    where TContext : class, ITransactionContext<string, TContext>, new()
    where TRequest : ISagaTransactiveRequest<TContext, TResponse>
{
    public override Type RequestType => typeof(TRequest);

    protected MediatRTransactionalSagaStep(string identifier, RollbackPolicy rollbackPolicy = RollbackPolicy.RollbackToCurrentStep) : base(identifier, rollbackPolicy)
    {
    }
}
